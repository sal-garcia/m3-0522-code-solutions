...
export default class App extends React.Component {
  ...
  toggleCompleted(todoId) {
    ...
    fetch(`/api/todos/${todoId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ isCompleted: !this.state.todos[todoIndex].isCompleted })
    }).then(res => res.json())
      .then(data => {
        const newList = this.state.todos.slice(0, todoIndex).concat([data], this.state.todos.slice(todoIndex + 1));
